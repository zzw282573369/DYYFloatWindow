...
//        [self makeKeyAndVisible];
        ...
        [_mainImageButton setFrame:CGRectMake(0, 0,frame.size.width, frame.size.height)];
...
{
    int i = 0;
    NSLog(@"key: %@", [_imagesAndTitle allKeys]);
    
    for (NSString *key in _imagesAndTitle)
    {
        ChildButton *childBtn = [[ChildButton alloc] initWithFrame:CGRectMake((self.frameWidth+marginWith) * i , 0, self.frameWidth , self.frameWidth )];
//        childBtn.backgroundColor = [UIColor redColor];
        UIImage * image = [UIImage imageNamed:key];
//        [childBtn setBackgroundImage:image forState:UIControlStateNormal];
        [childBtn setImage:image forState:UIControlStateNormal];
        [childBtn setTitle:_imagesAndTitle[key] forState:UIControlStateNormal];
        
        [childBtn addTarget:self action:@selector(itemsClick:) forControlEvents:UIControlEventTouchUpInside];
//        childBtn.hidden = YES;
//        childBtn.alpha = 0.8;
        childBtn.tag = i;
        
        [self.contentView addSubview:childBtn];
        i++;
    };
}
    ...
    if (!self.isShowTab)
    {
        ...
        [UIView animateWithDuration:showDuration animations:^{
            ...
            if (self.frame.origin.x <= kScreenWidth/2)
            {
                ...
                self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, WIDTH + _imagesAndTitle.count * (self.frameWidth + marginWith + 5) ,self.frameWidth);
                
                NSLog(@"AAAAAAAAAA %f : %f", self.frame.origin.x, self.frame.origin.y);
            ...
            else
            {
                ...
                self.mainImageButton.frame = CGRectMake((_imagesAndTitle.count * (self.frameWidth + marginWith + 5)), 0, self.frameWidth, self.frameWidth);
                
                self.frame = CGRectMake(self.frame.origin.x  - _imagesAndTitle.count * (self.frameWidth + marginWith + 5), self.frame.origin.y, WIDTH + _imagesAndTitle.count * (self.frameWidth + marginWith + 5) ,self.frameWidth);
                
                NSLog(@"BBBBBBBBBB %f : %f", self.frame.origin.x, self.frame.origin.y);
    ...
    else
    {
        ...
        [UIView animateWithDuration:showDuration animations:^{
            ...
            else
            {
                ...
                self.frame = CGRectMake(self.frame.origin.x + _imagesAndTitle.count * (self.frameWidth + marginWith + 5), self.frame.origin.y, self.frameWidth ,self.frameWidth);
            }
            self.backgroundColor = [UIColor clearColor];
        }];
...
- (void)moveContentviewLeft
{
    ...
    _contentView.frame = CGRectMake(WIDTH - _imagesAndTitle.count * (_frameWidth + marginWith) - (_frameWidth + 5), 0 ,_imagesAndTitle.count * (_frameWidth + marginWith), _frameWidth);
}
...
{
    NSLog(@"555555555");
    _contentView.frame = CGRectMake(_frameWidth + 5, 0, _imagesAndTitle.count * (_frameWidth + marginWith), _frameWidth);
}
